...
export const REGIONS: RegionDef[] = [
  {
    ...
    objects: [
      { id: 'proj-digger',         label: 'INDEX-DIGGER',           type: 'planet',   description: 'System query frequency: elevated. Log size: 1.4TB. Method of categorization: unresolved.', lore: 'Index tracking continues. Query logs show requests without source addresses.', region: 'projects', worldId: 12, worldPortal: 'newspaper', position: [1600, 400, -80],   color: '#60A5FA', size: 28, visible: 'always' },
      { id: 'proj-website',        label: 'RECURSIVE-∅',            type: 'anomaly',  description: 'Self-referencing structure. Nested node count: unstable. Integrity check: suspended.', lore: 'Recursive loops detected in segment headers. Readouts match system root.', region: 'projects', worldId: 12, worldPortal: 'newspaper', position: [1750, 520, -120],  color: '#93C5FD', size: 18, visible: 'always' },
      { id: 'proj-faraday',        label: 'FARADAY-RECEPTOR',       type: 'planet',   description: 'Signal source unresolved. Arrival timestamp predates index establishment. Solar load: critical.', lore: 'No establishment log. Sensor records trace energy fluctuations to 2018.', region: 'projects', worldId: 12, worldPortal: 'newspaper', position: [1420, 280, -70],    color: '#1D4ED8', size: 22, visible: 'always' },
      { id: 'proj-hidden-station', label: 'NODE-309',               type: 'station',  description: 'Decommissioned database relay. 47 archives remain unprocessed. Access restricted.', lore: 'Station offline since 2019. Diagnostic output: 47 files pending index.', region: 'projects', worldId: 5, worldPortal: 'cursor-flood', position: [1850, 320, -150], color: '#BFDBFE', size: 12, visible: { visited: 2 } },
      { id: 'proj-endpoint',       label: 'TERMINATION-POINT',      type: 'signal',   description: 'Final telemetry coordinates before carrier wave collapse. Signal level: -94dB.', lore: 'Transmission terminated. Telemetry log ends at coordinate boundary.', region: 'projects', worldId: 7, worldPortal: 'expand-white', position: [1980, 200, -60],   color: '#E5E5E5', size: 7,  visible: { visited: 6 } },
      { id: 'proj-loop',           label: 'SYSTEM-LOOP-10',         type: 'wormhole', description: 'Aperture dimension matches internal sequence. Geometry: non-Euclidean.', lore: 'Index coordinates redirect here. TRAVERSE WITH EXTREME CAUTION.', region: 'projects', worldId: 8, worldPortal: 'vortex', position: [1680, 290, -110],   color: '#818CF8', size: 30, visible: { visited: 4 } },
    ]
  },
  {
    id: 'running',
  ...
  },
  {
    ...
    objects: [
      { id: 'arch-frag-1',    label: 'DATA-MEM-001',        type: 'fragment', description: 'Recovered index block. System header matches master log. Format: corrupted.', region: 'archives', worldId: 8, worldPortal: 'rotate', position: [-1310, 1420, -280],  color: '#92400E', size: 10, visible: 'always' },
      { id: 'arch-frag-2',    label: 'DATA-MEM-047',        type: 'fragment', description: 'Fossilized code block. Creation date: 2020. Read access: unrestricted.', region: 'archives', worldId: 8, worldPortal: 'rotate', position: [-1090, 1180, -320], color: '#78350F', size: 10, visible: 'always' },
      ...
      { id: 'arch-frag-3',    label: 'DATA-MEM-113',        type: 'fragment', description: 'Orphan data fragment. Internal pointer is broken. Arrived via out-of-band transmission.', region: 'archives', worldId: 8, worldPortal: 'rotate', position: [-1160, 1410, -290],  color: '#92400E', size: 8,  visible: { visited: 3 } },
    ]
  },
  {
    id: 'explore',
    ...
    objects: [
      ...
      { id: 'explore-spiral',    label: 'DESCENT-DESCENT',  type: 'wormhole', description: 'Vertical gravity tube. Downward velocity: compounding. Bottom contact: undetected.', lore: 'Sensor probe dropped into tube 18 months ago. Still transmitting.', region: 'explore', worldId: 10, worldPortal: 'vortex', position: [1260, -1620, 220],   color: '#6366F1', size: 40, visible: { visited: 1 } },
    ]
  },
  {
    id: 'lab',
    ...
    objects: [
      { id: 'lab-collider',  label: 'PARTICLE-01',      type: 'anomaly',  description: 'High energy simulation grid. Particle count: 2000. Density: variable.', lore: 'Simulation responds to visual cursor. Heat output: elevated.', region: 'lab', worldId: 13, worldPortal: 'expand-white', position: [100, 1800, -230],    color: '#A855F7', size: 22, visible: 'always' },
      { id: 'lab-synth',     label: 'COLOR-SYNTH',      type: 'anomaly',  description: 'Generative visual matrix. Wavelength range: 380-740nm. State: unstable.', lore: 'Mouse coordinate input directly distorts spectral output.', region: 'lab', worldId: 13, worldPortal: 'expand-white',      position: [220, 1920, -280],  color: '#C084FC', size: 18, visible: 'always' },
      { id: 'lab-distort',   label: 'GRID-DISTORTION',  type: 'anomaly',  description: 'Local metric distortion. Space lattice display: warped. Access risk: high.', lore: 'Grid displays tears at outer boundaries. Gravity index fluctuating.', region: 'lab', worldId: 13, worldPortal: 'expand-white',       position: [-40, 1680, -210],   color: '#7C3AED', size: 16, visible: 'always' },
      { id: 'lab-collider2', label: 'COLLIDER-02',      type: 'anomaly',  description: 'Language fragment collision chamber. Diagnostic log: buffer overflow.', lore: 'Fragment collisions generate garbage logs at 440kb/s.', region: 'lab', worldId: 13, worldPortal: 'expand-white',        position: [180, 1720, -290],  color: '#DDD6FE', size: 14, visible: 'always' },
      ...
      { id: 'lab-index',     label: 'MASTER-CATALOG',   type: 'station',  description: 'Unsorted document index. Sort order: unverified. Total file count: disputed.', lore: 'Root directory displays 16 sub-directories. 1 is hidden.', region: 'lab', worldId: 11, worldPortal: 'fold',    position: [-180, 1840, -170],  color: '#818CF8', size: 22, visible: { visited: 5 } },
      { id: 'lab-void',      label: 'NULL-EXPERIMENT',  type: 'fragment', description: 'Terminated laboratory script. Output: partial. Log file: missing.', lore: 'Experiment terminated by system administrator. Reason: heap exhaustion.', region: 'lab', worldId: 13, worldPortal: 'expand-white', position: [-110, 1960, -270],  color: '#7C3AED', size: 9,  visible: { visited: 4 } },
    ]
  },
  {
    id: 'forgotten',
...
const PORTAL_GATES: Array<Omit<UniverseObject, 'type' | 'region' | 'visible'>> = [
  ...
  { id: 'gate-05', label: 'COMMERCE-05',description: 'Commerce zone. Operational status: closed. Infrastructure: intact.',   lore: 'PA system still running. No staff.',                         worldId: 5,  worldPortal: 'cursor-flood', position: [-736, -76,  48],  color: '#cc9966', size: 18 },
  ...
  { id: 'gate-08', label: 'DARKROOM-08', description: 'Chemical environment. Luminosity: minimal. Safe light active.',       lore: 'Timing is everything. Pull too early or too late.',         worldId: 8, worldPortal: 'vortex',       position: [812, -588, -35], color: '#8B0000', size: 20 },
  { id: 'gate-09', label: 'MOTH-09',    description: 'Light source mobile. Structure interior: unlit. Object count: several.', lore: 'The moth follows the light.',                               worldId: 9, worldPortal: 'nothing',      position: [611,  844, 28],  color: '#c8a850', size: 20 },
  { id: 'gate-10', label: 'SKY-10',     description: 'Navigable star field. Named clusters: 7. One unregistered.',            lore: 'Some constellations are personal.',                          worldId: 10, worldPortal: 'scatter',      position: [67,   1027, -56], color: '#A78BFA', size: 20 },
  { id: 'gate-11', label: 'ATTIC-11',   description: 'Storage space. Contents: artifacts from other worlds. One locked.',     lore: 'The number is somewhere in the system.',                    worldId: 11, worldPortal: 'fold',         position: [-780, -627, -28], color: '#94A3B8', size: 20 },
