...
export const REGIONS: RegionDef[] = [
  {
    ...
    objects: [
      { id: 'proj-digger',         label: 'INDEX-DIGGER',           type: 'planet',   description: 'System query frequency: elevated. Log size: 1.4TB. Method of categorization: unresolved.', lore: 'Index tracking continues. Query logs show requests without source addresses.', region: 'projects', worldId: 17, worldPortal: 'newspaper', position: [1600, 400, -80],   color: '#60A5FA', size: 28, visible: 'always' },
      { id: 'proj-website',        label: 'RECURSIVE-∅',            type: 'anomaly',  description: 'Self-referencing structure. Nested node count: unstable. Integrity check: suspended.', lore: 'Recursive loops detected in segment headers. Readouts match system root.', region: 'projects', worldId: 17, worldPortal: 'newspaper', position: [1750, 520, -120],  color: '#93C5FD', size: 18, visible: 'always' },
      { id: 'proj-faraday',        label: 'FARADAY-RECEPTOR',       type: 'planet',   description: 'Signal source unresolved. Arrival timestamp predates index establishment. Solar load: critical.', lore: 'No establishment log. Sensor records trace energy fluctuations to 2018.', region: 'projects', worldId: 17, worldPortal: 'newspaper', position: [1420, 280, -70],    color: '#1D4ED8', size: 22, visible: 'always' },
      { id: 'proj-hidden-station', label: 'NODE-309',               type: 'station',  description: 'Decommissioned database relay. 47 archives remain unprocessed. Access restricted.', lore: 'Station offline since 2019. Diagnostic output: 47 files pending index.', region: 'projects', worldId: 7, worldPortal: 'cursor-flood', position: [1850, 320, -150], color: '#BFDBFE', size: 12, visible: { visited: 2 } },
  ...
  },
  {
    ...
    objects: [
      { id: 'run-pikes',   label: 'PEAK-14115',         type: 'planet',   description: 'Geological spire. Recorded altitude: 14,115ft. Local gravity variance: +1.2%.', lore: 'Atmospheric density falls below baseline. Spire structures suggest artificial carving.', region: 'running', worldId: 19, worldPortal: 'slide-right',   position: [-1500, -800, 230],  color: '#F97316', size: 30, visible: 'always' },
      { id: 'run-boulder', label: 'METRIC-26.2',        type: 'planet',   description: 'Vector acceleration tracking node. Completion coefficient: 3.4122. Status: constant.', lore: 'Telemetry drift detected. Coordinates autocorrecting to zero-point reference.', region: 'running', worldId: 19, worldPortal: 'slide-right', position: [-1380, -960, 180], color: '#FB923C', size: 22, visible: 'always' },
      { id: 'run-golden',  label: 'SEGMENT-GOLDEN',     type: 'fragment', description: 'Eroded physical pathway. Linear length: unverified. Visual markers: obscured.', region: 'running', worldId: 19, worldPortal: 'slide-right',  position: [-1620, -720, 210],   color: '#FED7AA', size: 18, visible: 'always' },
      { id: 'run-signal',  label: 'SIGNAL-02-B',        type: 'signal',   description: 'Active beacon. Cycle: 14.8 seconds. Source depth: unresolved.', lore: 'Continuous sweep pattern detected. Data payload empty.', region: 'running', worldId: 5, worldPortal: 'rotate', position: [-1480, -980, 250],  color: '#F97316', size: 8,  visible: { visited: 2 } },
      ...
      { id: 'run-season',  label: 'TEMPORAL-2025',      type: 'anomaly',  description: 'Incomplete data block. Year designation: pending verification. Classification withheld.', region: 'running', worldId: 19, worldPortal: 'slide-right', position: [-1680, -920, 160], color: '#FDBA74', size: 15, visible: { visited: 3 } },
    ]
  },
  {
    id: 'archives',
    ...
    objects: [
      { id: 'arch-core',      label: 'ARCHIVE-CORE',        type: 'station',  description: 'Master storage structure. Core temperature: absolute zero. Integrity check: 42% failed.', lore: 'Structure integrity compromised. Automated index queries return recursive errors.', region: 'archives', worldId: 6, worldPortal: 'nothing', position: [-1200, 1300, -300],   color: '#B45309', size: 25, visible: 'always' },
      { id: 'arch-frag-1',    label: 'DATA-MEM-001',        type: 'fragment', description: 'Recovered index block. System header matches master log. Format: corrupted.', region: 'archives', worldId: 10, worldPortal: 'rotate', position: [-1310, 1420, -280],  color: '#92400E', size: 10, visible: 'always' },
      { id: 'arch-frag-2',    label: 'DATA-MEM-047',        type: 'fragment', description: 'Fossilized code block. Creation date: 2020. Read access: unrestricted.', region: 'archives', worldId: 10, worldPortal: 'rotate', position: [-1090, 1180, -320], color: '#78350F', size: 10, visible: 'always' },
      ...
      { id: 'arch-frag-3',    label: 'DATA-MEM-113',        type: 'fragment', description: 'Orphan data fragment. Internal pointer is broken. Arrived via out-of-band transmission.', region: 'archives', worldId: 10, worldPortal: 'rotate', position: [-1160, 1410, -290],  color: '#92400E', size: 8,  visible: { visited: 3 } },
    ]
  },
  {
    id: 'explore',
    ...
    objects: [
      { id: 'explore-mt-elbert', label: 'PEAK-14439',       type: 'planet',   description: 'Massive geological structure. Highest elevation on survey path. Barometric readout: missing.', lore: 'Summit coordinates confirmed. No traces of organic life found at elevation.', region: 'explore', worldId: 20, worldPortal: 'scatter', position: [1200, -1300, 180],   color: '#22C55E', size: 32, visible: 'always' },
      { id: 'explore-maroon',    label: 'MAROON-RELAY',     type: 'station',  description: 'Survey instrumentation array. Signal output: static. Solar panel condition: degraded.', region: 'explore', worldId: 20, worldPortal: 'scatter', position: [1320, -1180, 130],  color: '#4ADE80', size: 20, visible: 'always' },
      { id: 'explore-uncharted', label: 'SECTOR-UNMAPPED',  type: 'anomaly',  description: 'No records exist. Sensor sweep returns inconsistent distances. Geometry: shifting.', region: 'explore', worldId: 20, worldPortal: 'scatter', position: [1060, -1420, 200],   color: '#86EFAC', size: 16, visible: { visited: 2 } },
      { id: 'explore-hidden',    label: 'BEACON-04-Δ',      type: 'signal',   description: 'Low power beacon. Pulse frequency: irregular. Transmission: binary sequence.', region: 'explore', worldId: 20, worldPortal: 'scatter', position: [1350, -1320, 110],  color: '#22C55E', size: 8,  visible: { time: 20000 } },
  ...
  },
  {
    ...
    objects: [
      { id: 'lab-collider',  label: 'PARTICLE-01',      type: 'anomaly',  description: 'High energy simulation grid. Particle count: 2000. Density: variable.', lore: 'Simulation responds to visual cursor. Heat output: elevated.', region: 'lab', worldId: 18, worldPortal: 'expand-white', position: [100, 1800, -230],    color: '#A855F7', size: 22, visible: 'always' },
      { id: 'lab-synth',     label: 'COLOR-SYNTH',      type: 'anomaly',  description: 'Generative visual matrix. Wavelength range: 380-740nm. State: unstable.', lore: 'Mouse coordinate input directly distorts spectral output.', region: 'lab', worldId: 18, worldPortal: 'expand-white',      position: [220, 1920, -280],  color: '#C084FC', size: 18, visible: 'always' },
      { id: 'lab-distort',   label: 'GRID-DISTORTION',  type: 'anomaly',  description: 'Local metric distortion. Space lattice display: warped. Access risk: high.', lore: 'Grid displays tears at outer boundaries. Gravity index fluctuating.', region: 'lab', worldId: 18, worldPortal: 'expand-white',       position: [-40, 1680, -210],   color: '#7C3AED', size: 16, visible: 'always' },
      { id: 'lab-collider2', label: 'COLLIDER-02',      type: 'anomaly',  description: 'Language fragment collision chamber. Diagnostic log: buffer overflow.', lore: 'Fragment collisions generate garbage logs at 440kb/s.', region: 'lab', worldId: 18, worldPortal: 'expand-white',        position: [180, 1720, -290],  color: '#DDD6FE', size: 14, visible: 'always' },
      ...
      { id: 'lab-echo',      label: 'NODE-ECHO',        type: 'station',  description: 'Diagnostic monitor. Output channel: 88.7 MHz. Acknowledgment status: pending.', lore: 'FM receiver picked up loop sequence. Transmitter power: 50W.', region: 'lab', worldId: 8, worldPortal: 'fold', position: [300, 1800, -220],   color: '#E879F9', size: 12, visible: { time: 30000 } },
      ...
      { id: 'lab-void',      label: 'NULL-EXPERIMENT',  type: 'fragment', description: 'Terminated laboratory script. Output: partial. Log file: missing.', lore: 'Experiment terminated by system administrator. Reason: heap exhaustion.', region: 'lab', worldId: 18, worldPortal: 'expand-white', position: [-110, 1960, -270],  color: '#7C3AED', size: 9,  visible: { visited: 4 } },
    ]
  },
  {
    id: 'forgotten',
